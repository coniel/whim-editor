...
$ yarn init
yarn init v1.7.0
question name (input): @braindrop-editor/input
...
```bash
yarn workspace @braindrop-editor/input add -P react styled-components
```
...
Changes:
 - @braindrop-editor/input: 0.0.0 => 0.1.0
...
lerna WARN ENOLICENSE Packages @braindrop-editor/input, @braindrop-editor/login-form are missing a license
lerna notice
lerna notice 📦  @braindrop-editor/input@0.1.0
...
lerna notice name:          @braindrop-editor/input
...
lerna info published @braindrop-editor/input 0.1.0
Successfully published:
 - @braindrop-editor/input@0.1.0
...
```bash
$ yarn info @braindrop-editor/input
yarn info v1.10.1
{ name: '@braindrop-editor/input',
  versions:
   [ '0.1.0' ],
  'dist-tags':
   { latest: '0.1.0' },
...
Let's create second package which is dependant on our `@braindrop-editor/input` and check how our tools works with it.

### Development

Just create second package in the same way as the first one. In our example it would be `@braindrop-editor/login-form` with next content:

```typescript
import { Input } from '@braindrop-editor/input';
import * as React from 'react';
import styled from 'styled-components';
...
Now we could add `@braindrop-editor/input` to the list of dependencies. This could be achieved easily with next command:

```bash
yarn lerna add @braindrop-editor/input --scope=@braindrop-editor/login-form
```
...
yarn run v1.10.1
$ tsc
packages/login-form/src/LoginForm.tsx:1:23 - error TS2307: Cannot find module '@braindrop-editor/input'.

1 import { Input } from '@braindrop-editor/input';
                        ~~~~~~~~~~~~~
error Command failed with exit code 1.
```

It happens because in the `package.json` file for `@braindrop-editor/input` package we have `"main": "dist/index.js"` which is pointing to the build version of the package and we have not run `build` command. So, let's build our packages and check `tsc` again:

```bash
$ yarn tsc
yarn run v1.10.1
$ tsc
packages/login-form/src/LoginForm.tsx:1:23 - error TS7016: Could not find a declaration file for module '@braindrop-editor/input'. 'monorepo-babel-ts/packages/input/dist/index.js' implicitly has an 'any' type.
  Try `npm install @types/taxi__input` if it exists or add a new declaration (.d.ts) file containing `declare module '@braindrop-editor/input';`
...
Now we could check that `@braindrop-editor/login-form` works as well, just return it back to the `packages` directory and check that `tsc` works now.
...
We are able to generate declarations for all packages, run `storybook` and `tests`, however, as soon as we change `@braindrop-editor/input` we would not see any changes in the `storybook` for `@braindrop-editor/login-form` because we still would use previously built version. To see these changes we would need to rebuild packages one more time. The same applies to the `jest` runs.
...
`"@braindrop-editor/*": ["packages/*/src"]` tells to `tsc` where to search for `@braindrop-editor/` packages. In our case we would like to search them in `packages/*/src` folders as there we have our source code.
...
Module not found: Error: Can't resolve '@braindrop-editor/input' in 'monorepo-babel-ts/packages/login-form/src'
@ ./packages/login-form/src/LoginForm.tsx 8:13-35
```

It happens even when `tsc` could find those modules. The reason of this is simple: `tsc` checks files, `babel` transforms them and tries to execute, and it doesn't know anything about `@braindrop-editor/input` as `main` still points to the `dist` folder.
...
Now we could start storybook and check that it works, moreover, if we change `@braindrop-editor/input` and check stories for `@braindrop-editor/login-form` they would have changes immediately without even building packages.
...
```bash
 FAIL  packages/login-form/src/LoginForm.test.tsx
  ● Test suite failed to run
    ...
    Cannot find module '@braindrop-editor/input' from 'LoginForm.test.tsx'
```
...
`'@braindrop-editor/(.+)$': '<rootDir>packages/$1/src` will tell jest where to find source code for `@braindrop-editor/` packages and again it points to the `src` instead of `dist`.
